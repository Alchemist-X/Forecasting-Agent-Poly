---
name: daily-pulse
description: "在 forecasting-agent-poly 仓库中运行 daily pulse 主流程。用于用户要求执行 daily pulse、生成当日组合 review / monitor / rebalance 报告，或明确说“运行 daily pulse”时。默认主链路是 live real-money，并优先使用 .env.primary 作为测试钱包环境；只有用户明确要求预览或只看建议时，才降级到 --recommend-only。"
---

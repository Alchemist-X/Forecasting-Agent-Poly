...
```bash
# 实盘（默认）
ENV_FILE=.env.primary pnpm pulse:live

# 只看推荐不下单
ENV_FILE=.env.primary pnpm pulse:recommend

# 指定分类
ENV_FILE=.env.primary pnpm pulse:live -- --category politics

# 复用已有 pulse 快照
ENV_FILE=.env.primary pnpm pulse:live -- --pulse-json runtime-artifacts/reports/pulse/.../pulse-xxx.json
```
